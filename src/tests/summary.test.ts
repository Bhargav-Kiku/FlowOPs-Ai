...
describe("POST /api/v1/summary", () => {
  ...
  test("returns 200 with a summary string", async () => {
    mockCreate.mockResolvedValueOnce(
      mockGeminiResponse({ summary: "Guest Jane Smith in room 408 has requested extra pillows and a late checkout on August 1st." })
    );
  ...
  test("uses the fast model for summary (check _meta.model)", async () => {
    mockCreate.mockResolvedValueOnce(
      mockGeminiResponse({ summary: "Test summary." })
    );
    ...
    expect(res.status).toBe(200);
    expect(res.body._meta.model).toBe(process.env.GEMINI_FAST_MODEL ?? "gemini-3.6-flash");
  });

  test("accepts various context shapes (flexible input)", async () => {
    mockCreate.mockResolvedValueOnce(
      mockGeminiResponse({ summary: "Property had 45 requests with 92% SLA compliance." })
    );
  ...
  test("always injects ai_role: recommendation_only", async () => {
    mockCreate.mockResolvedValueOnce(
      mockGeminiResponse({ summary: "A summary." })
    );
